...
for language in os.listdir('models_v1_0'):
    if language != 'all_langs':
        for configuration in os.listdir(f'models_v1_0/{language}'):
            for checkpoint in os.listdir(f'models_v1_0/{language}/{configuration}'):

                model = BertForTokenClassification.from_pretrained(f"models_v1_0/{language}/{configuration}/{checkpoint}")
                ...
                # Sample text
                for dataset in os.listdir(f'predictions_v1_0/{language}'):
                    if 'test' in dataset:
                        test_df = pd.read_csv(f"predictions_v1_0/{language}/{dataset}", sep="\t")
                        ...
                        for text in texts:
                            ...
                            for item in output:
                                word = item["word"]
                                if word.startswith("##") or word in ["[CLS]", "[SEP]"]:
                                    continue
                                label_index = int(item["entity"].split("_")[-1])
                                sentence_labels.append(id2label[label_index])

                            print(len(text.split()), len(sentence_labels))
                            assert len(text.split()) == len(sentence_labels), 'Something went wrong.'
                            all_labels.append(' '.join(sentence_labels))

                        test_df[configuration] = all_labels
                        test_df.to_csv(f"predictions_v1_0/{language}/{dataset}", index=False, sep="\t")
...
# Tokenizer (shared across all models_v1_0)
tokenizer = BertTokenizerFast.from_pretrained("bert-base-multilingual-cased")
max_len = tokenizer.model_max_length  # usually 512 for BERT

# Directory where the five “all_langs” models_v1_0 live
ALL_LANGS_DIR = "models_v1_0/all_langs"
...
for language in os.listdir("models_v1_0"):
    ...
    data_dir = f"predictions_v1_0/{language}"
    for dataset in os.listdir(data_dir):
        ...
        # save all predictions_v1_0 for this test file
